...
6. If the user explicitly requests a bounded campaign, use only the independent `sigilicon campaign plan/run` CLI after confirming owner opt-in; this read-only skill does not invoke campaigns by default. Check: no campaign call appears without that explicit request.
