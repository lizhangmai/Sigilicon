"""Declarative custom-IP release contracts and qualification rules."""
...
class IpContract:
    ...
    def require_level(self, level: str) -> str:
        if level not in QUALIFICATION_LEVELS:
            raise ValueError(
                f"unsupported qualification {level!r}; expected one of "
                f"{', '.join(QUALIFICATION_LEVELS)}"
            )
        return level
...
def load_ip_contract(path: Path, *, project_root: Path) -> IpContract:
    ...
    for index, value in enumerate(exports_raw):
        ...
        qualification = _table(
            entry.get("qualification"), f"exports[{index}].qualification"
        )
        required_roles: dict[str, tuple[str, ...]] = {}
        previous: set[str] = set()
        for level in QUALIFICATION_LEVELS:
            level_table = _table(
                qualification.get(level),
                f"exports[{index}].qualification.{level}",
            )
            ...
            if not isinstance(values, list) or any(
                ...
                raise ValueError(
                    f"exports[{index}].qualification.{level}.required_roles "
                    "must be strings"
                )
            current = set(values)
            ...
            if not previous.issubset(current):
                raise ValueError(
                    f"exports[{index}].qualification.{level} must include "
                    "lower-level roles"
                )
    ...
    if default not in QUALIFICATION_LEVELS:
        raise ValueError("default_qualification is unsupported")
    result = IpContract(
        path=contract_path,
        project_root=root,
        name=ip_name,
