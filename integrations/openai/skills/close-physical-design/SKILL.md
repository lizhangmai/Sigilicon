...
6. If a stage is violated and typed attribution is available, prepare the smallest owner-policy repair proposal. Sigilicon has no bundled placement, routing, or physical-closure repair implementation; continue only after the owner supplies a new identity-bound physical result and target plan, then rerun all owner-required regressions. Check: the replacement result and immutable plan identities are recorded.
7. OA-XStream and combined XStream-Calibre are explicit owner-selected Adapters. If the user explicitly requests a bounded campaign, use only `sigilicon campaign plan/run`; this target workflow does not call a campaign by default. Check: owner selection and the explicit campaign request are recorded before use.
