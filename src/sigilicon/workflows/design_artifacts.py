...
class DesignArtifactInterface:
    ...
    def validate_decision(
        ...
        decision = design_artifact_from_json(decision_json)
        candidate = design_artifact_from_json(candidate_json)
        evidences = tuple(design_artifact_from_json(text) for text in evidence_json)
        if not isinstance(decision, DesignDecision):
            raise ValueError("decision input is not a Design Decision")
        if not isinstance(candidate, DesignCandidate):
            raise ValueError("candidate input is not a Design Candidate")
        if any(not isinstance(item, DesignEvidence) for item in evidences):
            raise ValueError("decision evidence input contains a non-evidence artifact")
        validate_design_decision(
            decision,
            candidate,
            tuple(item for item in evidences if isinstance(item, DesignEvidence)),
        )
