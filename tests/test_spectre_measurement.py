...
config = {{ program = "measure.py", spec = "spec.toml", circuit = "circuit.scs", platform = "testpdk", model_set = "nominal", parameters = {{ mode = "{mode}" }}, timeout_seconds = 10 }}
evidence = {{ role = "regression", level = "l1", scope = "fixture" }}
''')
    component = write_component_owner(tmp_path, "fixture", filesets={
        "measurement": tuple(f"ip/fixture/{name}" for name in ("operations.toml", "measure.py", "spec.toml", "circuit.scs")),
    })
