...
def _feedback(
    ...
    if not plan.executable and not scopes:
        scopes.append(
            ClosureFeedbackScope(
                ClosureFeedbackKind.MATERIALIZATION,
                (plan.acceptance.reason.value,),
...
class ClosureCampaignRunner:
    ...
    def _observe(
        ...
        for label, reference, kind in (
            ...
            if reference is not None:
                artifacts[label] = _artifact(flow_result, reference, kind)
        ...
        if "layout" in artifacts:
            _expect_qualifier(
                artifacts["layout"], "plan-sha256", plan_sha256_value, issues
            )
            _expect_qualifier(
                artifacts["layout"], "result-sha256", result_sha256, issues
            )
            _expect_qualifier(
                artifacts["layout"], "owner", plan.target.owner, issues
            )
            _expect_qualifier(
                artifacts["layout"], "name", plan.target.name, issues
            )
        ...
        for label, evidence in (("DRC", drc), ("LVS", lvs)):
            ...
            _expect_qualifier(
                evidence_artifact,
                "layout-sha256",
                evidence.layout.artifact_sha256,
                issues,
            )
            _expect_qualifier(
                evidence_artifact,
                "plan-sha256",
                evidence.layout.plan_sha256,
                issues,
            )
            _expect_qualifier(
                evidence_artifact,
        ...
        return _ObservedClosure(
            ...
            lvs,
            quality,
            _feedback(closure, plan, drc, lvs),
